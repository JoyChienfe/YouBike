  ...
  <div class="row mt-4">
    ...
    <div class="col-6">
      <n-date-picker
        ...
        class="mb-2"
      />
      <n-button type="primary" @click="exportfun1">匯出</n-button>
    </div>
    <n-divider />
  </div>
</template>
...
function makedata1(data, time) {
  ...
  data.forEach((item) => {
    ...
    //先判斷是否一級檢修大於30天
    const isMore30 = isGreaterThanDatePlusDays(
      range.value[0],
      range.value[1],
      item.testtable.date,
      30
    ...
    //如果只要有一個o 跟超過30天就 為異常車
    if (arr.includes("o") || isMore30) {
      arr.push("o");
    }
...
function makedata3(data, time) {
  ...
  data.forEach((item) => {
    const arr = [
      ...
      formateDate(item, "電量", "電量低於30系統未鎖車"),
      item.testtable.date,
      item.testtable.testman,
...
//判斷兩個時間戳中的最小值是否大於指定日期加上特定天數後的時間。
function isGreaterThanDatePlusDays(
  timestamp1,
  timestamp2,
  referenceDateString,
  days
  ...
  // 將參考日期字符串轉換為時間戳
  const referenceDate = new Date(referenceDateString).getTime();

  // 計算參考日期加上指定天數後的時間戳
  const datePlusDays = referenceDate + days * 86400000;

  // 找到兩個時間戳中的最小值
  const minTimestamp = Math.min(timestamp1, timestamp2);

  // 判斷最小的時間戳是否大於參考日期加上指定天數後的時間戳
  return minTimestamp > datePlusDays;
}
</script>
