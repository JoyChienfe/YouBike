...
import drawerDark from "../components/station/stationDrawComponent.vue";
...
async function deleteData(id, sys) {
  let url = null;
  if (sys === "1.0") {
    url = `${import.meta.env.VITE_URL}/station/station1/${id}`;
  } else {
    url = `${import.meta.env.VITE_URL}/station/station2/${id}`;
  }
  try {
...
const columns = createColumns({
  check(row) {
    ...
    dataAll.value.forEach((item) => {
      if (item.id === row["f_id"] && item.sys === row["car_type"]) {
        formData.value = item;
      }
    });
  ...
  async dele(row) {
    try {
      ...
      if (res) {
        const { f_id, car_type } = row;
        await deleteData(f_id, car_type);
        await getData();
      }
    } catch (error) {
      console.log("刪除失敗");
...
async function getData() {
  try {
    ...
    const ststion1_url = `${import.meta.env.VITE_URL}/station/station1`;
    const ststion2_url = `${import.meta.env.VITE_URL}/station/station2`;
    const users_url = `${import.meta.env.VITE_URL}/user/users`;
    //場站資料
    const res = await Promise.all([
      axios.get(ststion1_url),
      axios.get(ststion2_url),
      axios.get(users_url),
    ]);
    const data1 = res[0]["data"];
    const data2 = res[1]["data"];
    isLoading.value = false;
    dataAll.value = [...data1, ...data2];
    fromdata.value = dataAll.value.map((item) => {
      return {
        ...
        car_type: item.sys,
        city: item.station_name.split("-")[0],
      };
    });
  } catch (error) {
    console.log(error);
