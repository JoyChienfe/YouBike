...
const ruleFormAll = ref<BiketestStepSeven>({
  catequery: "1.0",
  startrow: null,
  endrow: null,
  text: "",
...
const rulesAll = ref<FormRules<BiketestStepSeven>>({
...
import type { BiketestStepSeven } from "env";
...
onMounted(() => {
  ...
  scrollToTop();
  //判斷有沒有該網站step 沒有就踢掉
  if (!teststore.step.step7) {
    router.push("/server/biketest/BiketestStepsix");
  }
...
const SetTempData = () => {
  ...
  if (nowSys === "1.0") {
    ...
    ruleFormAll.value.checkdata[1]["ischeck"] = teststore.testfrom2.煞車功能
      ?.ischeck as [];
    ruleFormAll.value.checkdata[2]["ischeck"] = teststore.testfrom2.變速功能
      ?.ischeck as [];
    ruleFormAll.value.checkdata[3]["ischeck"] = teststore.testfrom2.隨車鎖
      ?.ischeck as [];
  } else if (nowSys === "2.0E") {
    console.log("現在系統是2.0E,賦予資料中...");
    ruleFormAll.value.checkdata[4]["ischeck"] = teststore.testfrom3.輪胎
      ?.ischeck as [];
    ruleFormAll.value.checkdata[5]["ischeck"] = teststore.testfrom3.前後輪軸螺絲
      ?.ischeck as [];
    ruleFormAll.value.checkdata[6]["ischeck"] = teststore.testfrom3[
      "前／後燈(行進車輪轉動中)"
    ]?.ischeck as [];
    ruleFormAll.value.checkdata[7]["ischeck"] = teststore.testfrom3[
      "前／後燈(暫停車輪靜止中)"
    ]?.ischeck as [];
  } else {
    console.log("現在沒有系統");
...
const submitForm = async (formEl: FormInstance | undefined) => {
  ...
  try {
    ...
    if (ruleFormAll.value.catequery === "1.0") {
      ...
      if (!res) return;
      teststore.testfrom1.騎乘行進 = ruleFormAll.value.checkdata[0];
      teststore.testfrom1.startrow = ruleFormAll.value.startrow;
      teststore.testfrom1.endrow = ruleFormAll.value.endrow;
      teststore.testfrom1.text = ruleFormAll.value.text;
      console.log("送出1.0");
      const newNameAtrr = teststore.testfrom1.station_name!.split("-");
      const payload = {
        ...
        carnum: teststore.testfrom1.carnum,
        carnum2: teststore.testfrom1.carnum2,
        tablejson: teststore.testfrom1,
      };
      ...
      if (!isTakePhoto) {
        ...
        uploadData.value = {
          payload: `(${nowMonth}-${nowDay})-${teststore.testfrom1.carnum}`,
        };
        openModel();
      }
      ...
      //表單
      teststore.testfrom2.煞車功能 = ruleFormAll.value.checkdata[1];
      teststore.testfrom2.變速功能 = ruleFormAll.value.checkdata[2];
      teststore.testfrom2.隨車鎖 = ruleFormAll.value.checkdata[3];
      console.log("送出2.0");
    } else {
      //表單
      teststore.testfrom3.輪胎 = ruleFormAll.value.checkdata[4];
      teststore.testfrom3.前後輪軸螺絲 = ruleFormAll.value.checkdata[5];
      teststore.testfrom3["前／後燈(行進車輪轉動中)"] =
        ruleFormAll.value.checkdata[6];
      teststore.testfrom3["前／後燈(暫停車輪靜止中)"] =
        ruleFormAll.value.checkdata[7];
      console.log("送出2.0E");
    }

    //第八頁權限開啟
    teststore.step.step8 = true;
